...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--json_path", type=Path, default=root / "data" / "n5_strict_results.json")
    p.add_argument("--out_path", type=Path, default=root / "figures" / "n5_top1_verification_en.png")
...
def main() -> None:
    ...
    if use_target_band:
        best = min(candidate_pool, key=lambda x: (abs(float(x["omega"]) - target_omega), -float(x["avg_selectivity"])))
    else:
        best = max(candidate_pool, key=lambda x: x["avg_selectivity"])
    K = np.array(best["K"], dtype=float)
    omega_best = float(best["omega"])
    ...
    ax_full.set_title(
        f"N={n_nodes} K#{best['kidx']} Amplitude-Frequency (Best: {best_ratio:.2f}x, source={source_tag})",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax_bar.set_title(f"Seed Robustness at Omega={omega_best:.2f} ({len(seeds)} Seeds, Full-Width)", fontsize=14, fontweight="bold")
